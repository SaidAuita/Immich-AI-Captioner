...
- **`ui_language`**: Язык графического интерфейса приложения (`"ru"`, `"en"`, `"de"`, `"es"`, `"fr"`, `"ja"`, `"pt"`, `"zh"`).
- **`caption_language`**: Язык заголовка и текстового описания сцены, генерируемого нейросетью VLM (по умолчанию `"ru"`).
- **`tags_language`**: Язык ключевых слов / тегов для поиска (по умолчанию `"en"` или `"ru"`). Раздельный выбор языков позволяет создавать русскоязычные описания фотографий и одновременно формировать теги на английском языке для максимальной совместимости с Adobe Lightroom, Capture One, фотостоками и международными поисковыми индексами.
